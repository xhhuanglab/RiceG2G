#perl SelectFarmCPUPeakByCor.pl /data/home/mjchen/rice/NAM_denovo_assemble/RiceG2G/ Plant_height
...
$traitname=$ARGV[0];
$input="./data/farmCPU/$traitname.FarmCPU.csv" or die "can't open $traitname.FarmCPU.csv";
open IN,"$input" or die "can't open $input";
while (<IN>){
	chomp();
	if($.>1){
		@line =split/\,/,$_;
		if($line[7]<=0.0000000167 and $line[7] ne "NA"){
			$line[0]=~s/\"//g;
			push @snp,$line[0]; 
...
open IN3,"./basic_data/Parent_unmiss.tped" or die "can't open ./basic_data/Parent_unmiss.tped";
while (<IN3>){
	chomp();
	if($_=~/\d+	(snp\d+)	0	\d+/){
		$key=$1;
...
foreach $i(1..12){
	open IN4,"./basic_data/chr$i-nam-Effect.snp";
	while (<IN4>){
		chomp();
		if($_=~/Chr(\d+)	(\d+)	(\S+)	(\S+)	/){
			$key="$1	$2";
...
open OUT,">$traitname.FarmCPU.peak" or die "can't open the output file";
print OUT "Chromosome	Position	-log10(Pvalue)	Parent_geno	1_genotype	2_genotype	Effect	SE\n";
foreach (0..$final_num-1){
	$key="$sort_final[$_][0]	$sort_final[$_][1]";
	print OUT "$sort_final[$_][0]	$sort_final[$_][1]	$sort_final[$_][2]	$sort_final[$_][3]	$ref{$key}	$alt{$key}	$sort_final[$_][4]	$sort_final[$_][5]\n";
}
close OUT;
