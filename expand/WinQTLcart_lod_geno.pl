#perl WinQTLcart_lod_geno.pl SH_Heading_date
...
foreach $NAM(1..15){
	...
	close IN;
	$chr=0;
	open IN1,"./data/WinQTLcart/N$NAM.bin.list.map" or die "can't open N$NAM.bin.list.map";
	while (<IN1>){
		chomp();
		$line=$_;
		if($line=~/chr(\d+)	(\S+)/){
...
$chr=1;$star=0;
open IN2,"./basic_data/Rice_MSUv7.gff3" or die "can't open Rice_MSUv7.gff3";
while (<IN2>){
	chomp();
...
close IN2;
open OUT,">$ARGV[0]_NAM_LOD_geno.info";
