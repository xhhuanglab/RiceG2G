...
open IN1,"./basic_data/all.locus_brief_info.7.0" or die "can't open all.locus_brief_info.7.0";
while (<IN1>){
	chomp();
	@line =split/\t+/,$_;
	if($line[0]=~/Chr(\d+)/){
...
close IN1;
open IN2,"./basic_data/house-keeping_gene.list" or die "house-keeping_gene.list";
while (<IN2>){
	chomp();
	if($.>1){
...
close IN2;
open IN3,"./basic_data/repressed_geno.list" or die "repressed_geno.list";
while (<IN3>){
	chomp();
	if($.>1){
...
close IN3;
open IN4,"./basic_data/Basic_Information_Arabidopsis_to_rice.txt" or die "can't open Basic_Information_Arabidopsis_to_rice.txt";
while (<IN4>){
	chomp();
	if($.>1){
		@Arabidopsis=split/\t+/,$_;
...
open RAP4,"./basic_data/Basic_Information_Arabidopsis_to_rice_rap.txt" or die "can't open Basic_Information_Arabidopsis_to_ricee_rap.txt";
while (<RAP4>){
	chomp();
	if($.>1){
		@Arabidopsis=split/\t+/,$_;
...
open IN5,"./basic_data/Rice_MSUv7.gff3" or die "can't open Rice_MSUv7.gff3";
while (<IN5>){
	chomp();
...
close IN5;
open IN1,"./basic_data/RAP-MSU_2021-11-11.txt" or die "can't open pair file";
while (<IN1>){
	chomp();
	@pair=split/\t+/,$_;
	if($pair[1] eq "None"){
...
close IN1;
open INRAP,"./basic_data/Rice_IRGSP-1.0.gff3" or die "can't open Rice_IRGSP-1.0.gff3";
while (<INRAP>){
	chomp();
...
open RNA,"./basic_data/combineRAPvsMSU_RNAseq.txt" or die "can't open RNA data";
while (<RNA>){
	chomp();
	if($.>1){
		@rna=split/\s+/,$_;
...
close RNA;
open IN10,"$ARGV[0]_NAM_LOD_geno.info" or die "can't open $ARGV[0]_NAM_LOD_geno.info";
while (<IN10>){
	chomp();
	if($.>1){
		@win=split/\s+/,$_; 
...
close IN10;
open RAP10,"$ARGV[0]_NAM_LOD_geno_rap.info" or die "can't open $ARGV[0]_NAM_LOD_geno.info";
while (<RAP10>){
	chomp();
	if($.>1){
		@win=split/\s+/,$_; 
...
close IN11;
$win_peaknum=0;
open IN12,"$ARGV[0].winqtlcart.peak" or die "can't open INput $ARGV[0].winqtlcart.peak";
while (<IN12>){
	chomp();
	@win_peak=split/\s+/,$_;
	$win_peaknum++;
...
close IN12;
open RAPID,"./basic_data/RAP-MSU_2021-11-11.txt";
while (<RAPID>){
	chomp();
	@rap=split/\t+/,$_;
	@msuid=();
...
close RAPID;
open ARPann,"./basic_data/Rice_IRGSP-1.0.gff3";
while (<ARPann>){
	if($_=~/ID=(\S+);Name=/){
		$id=$1;
	}
...
close ARPann;
open IN9,"$ARGV[0].FarmCPU.peak" or die "can't open farmCPU.info";
while (<IN9>){
	chomp();
	if($.>1){
		@farm=split/\s+/,$_;
...
close IN9;
open IN6,"./basic_data/SIFTtolerantScore_MSUvsRAP.txt" or die "can't open SIFTtolerantScore_MSUvsRAP.txt";
while (<IN6>){
	chomp();
	@sift=split/\s+/,$_;
	$key="$sift[0]	$sift[1]";
...
close IN6;
foreach $chr(1..12){
	$file='./basic_data/Chr'."$chr".'_combineSNPeffect_MSUvsRAP.txt';
...
open IN6,"./basic_data/SIFTtolerantScore_MSUvsRAP.txt" or die "can't open SIFTtolerantScore.info";
while (<IN6>){
	chomp();
	@sift=split/\s+/,$_;
	if($.==1){
...
close IN6;
$hang=0;
open NoclodingSNP,"./basic_data/combineRAPvsMSU_nocodingSNP.txt" or die "can't open combineRAPvsMSU_nocodingSNP.txt";
while (<NoclodingSNP>){
	chomp();
	$hang++;$mis=0;
	if($hang>1){
...
foreach $chr(1..12){
	$file='./basic_data/Chr'."$chr".'_combineINDELtolerantScore_MSUvsRAP.txt';
	open IN, "$file" or die "can't open $file";
	while (<IN>){
		chomp();
		@indel=split/\s+/,$_;
...
$hang=0;
open NocodingIndel,"./basic_data/combineRAPvsMSU_nocodingINDEL.txt" or die "can't open combineRAPvsMSU_nocodingINDEL.txt";
while (<NocodingIndel>){
	chomp();
	$hang++;
	if($hang>1){
...
foreach $chr(1..12){
	$file='./basic_data/NAM_Chr'."$chr".'combineRAPvsMSU_codingSV.txt';
	open IN8,"$file" or die "can't open $file";
	while (<IN8>){
		chomp();
		@sv=split/\t+/,$_;
		if($.>1){
...
$hang=0;
open NocodingSV,"./basic_data/combineRAPvsMSU_nocodingSV.txt";
while (<NocodingSV>){
	chomp();
	$hang++;
	if($hang>1){
