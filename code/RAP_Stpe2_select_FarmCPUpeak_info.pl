...
open IN1,"./basic_data/key_word.txt" or die "can't open key_word.txt";
while (<IN1>){
	chomp();
	if($_=~/(\S+):(.*);(.*)?/){
		$trait=$1;
...
print OUT "QTL	FarmCPU_pos	FarmCPU_-logP	MLM_INFO(Chr-pos-(-log10))	Geno	Geno_pos	SIFT_Score	INDEL_Score	SV_Score	Score_peak	Score_TE	Score_annotate	Score_expression	Score_match	Score\n";
open IN2,"RAP_$ARGV[0].FarmCPUpeak_info" or die "can't open $ARGV[0].FarmCPUpeak_info";
while (<IN2>){
	chomp();
	if($.>1){
		...
		print OUT "$line[0]	$line[1]	$line[2]	$line[3]	$line[5]	$line[6]	$line[13]	$line[14]	$line[15]";
		foreach (0..15){
		#	print OUT "$line[$_]	";
		}
